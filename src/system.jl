...
function System{T}(ss::PyObject) where T <: AbstractFloat
    models = [make_instance(ty, ss[name]) for (ty, name) in zip(System{T}.types, fieldnames(System))]
    System{T}(models...)
end
...
"""
Base.@inline function Ymatrix!(line::Line{T}, rows::Vector{Int64}, cols::Vector{Int64}, vals::Vector{Complex{Float64}},
                  y1, y2, y12, itap, itap2, itapconj) where T<: AbstractFloat
...
function calc_Yinj!(line::Line{T}, bus::Bus{T},
                    Ymat::SparseMatrixCSC{Complex{T}, Int64}, Vbus::Vector{Complex{T}}, Sbus::Vector{Complex{T}},
                    Pvec::Vector{T}, Qvec::Vector{T}) where T<: AbstractFloat
    @simd for i = 1:bus.n
