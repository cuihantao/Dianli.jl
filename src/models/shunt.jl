...
Base.@inline alloc_triplets(::Type{Shunt{T}}, n::N) where {T <: AbstractFloat, N <: Integer} = Triplets{T, N}(2n)


Base.@propagate_inbounds function push_triplets!(shunt::Shunt{T}, ::Type{Val{:serial}}) where T <: AbstractFloat
    ndev = shunt.n
    @simd for i = 1:ndev
        #  d resP / da
...
Base.@propagate_inbounds function push_triplets!(shunt::Shunt{T}, ::Type{Val{:threaded}}) where T <: AbstractFloat
    ndev = shunt.n
